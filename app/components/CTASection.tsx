import Image from "next/image";
import { Mail, Phone, MapPin } from "lucide-react";
import { ContactForm } from "./forms/ContactForm";

export function CTASection() {
  return (
    <section id="contacto" className="relative py-24 md:py-32 bg-[var(--color-brand-dark)]">
      <div className="absolute inset-0 z-0">
        <Image
          src="/images/planta-industrial-cartagena-cmp-sas.webp"
          alt="Planta industrial CMP SAS Cartagena"
          fill
          sizes="100vw"
          quality={75}
          className="object-cover opacity-20"
        />
        <div className="absolute inset-0 bg-gradient-to-b from-black/80 via-black/60 to-black/80" />
      </div>

      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="text-center mb-16">
          <span className="text-[var(--color-brand-orange)] font-bold tracking-wider uppercase text-sm mb-4 block">
            Contacto
          </span>
          <h2 className="text-3xl md:text-5xl font-extrabold text-white mb-4">
            ¿Listo para proteger tu infraestructura?
          </h2>
          <p className="text-lg text-gray-400 max-w-2xl mx-auto">
            Nuestro equipo de especialistas está disponible para asesorarte en tu próximo proyecto.
          </p>
        </div>

        <div className="grid lg:grid-cols-5 gap-10 max-w-6xl mx-auto">
          {/* Info Column */}
          <div className="lg:col-span-2 flex flex-col justify-between">
            <div className="space-y-8">
              <div className="flex items-start gap-4">
                <div className="w-12 h-12 rounded-xl bg-[var(--color-brand-orange)]/10 flex items-center justify-center shrink-0">
                  <Phone size={22} className="text-[var(--color-brand-orange)]" />
                </div>
                <div>
                  <h3 className="text-white font-semibold mb-1">Teléfono</h3>
                  <a href="tel:+573137804820" className="text-gray-400 hover:text-white transition-colors">
                    +57 313 780 4820
                  </a>
                </div>
              </div>

              <div className="flex items-start gap-4">
                <div className="w-12 h-12 rounded-xl bg-[var(--color-brand-orange)]/10 flex items-center justify-center shrink-0">
                  <Mail size={22} className="text-[var(--color-brand-orange)]" />
                </div>
                <div>
                  <h3 className="text-white font-semibold mb-1">Email</h3>
                  <a href="mailto:ventasservicios@cmpcoating.com.co" className="text-gray-400 hover:text-white transition-colors">
                    ventasservicios@cmpcoating.com.co
                  </a>
                </div>
              </div>

              <div className="flex items-start gap-4">
                <div className="w-12 h-12 rounded-xl bg-[var(--color-brand-orange)]/10 flex items-center justify-center shrink-0">
                  <MapPin size={22} className="text-[var(--color-brand-orange)]" />
                </div>
                <div>
                  <h3 className="text-white font-semibold mb-1">Ubicación</h3>
                  <p className="text-gray-400">Cartagena de Indias, Colombia</p>
                </div>
              </div>
            </div>
          </div>

          {/* Form Column */}
          <div className="lg:col-span-3">
            <div className="bg-white/[0.03] backdrop-blur-sm p-8 md:p-10 rounded-3xl border border-white/10">
              <h3 className="text-xl font-bold text-white mb-6">Envíanos un mensaje</h3>
              <ContactForm variant="dark" />
            </div>
          </div>
        </div>

        <div className="mt-10 flex flex-col sm:flex-row items-center justify-center gap-4 max-w-6xl mx-auto">
          <p className="text-gray-500 text-sm">¿Prefieres contacto directo?</p>
          <a
            href="https://api.whatsapp.com/send?phone=573137804820"
            className="bg-[#25D366] hover:bg-[#20BE5C] text-white px-6 py-3 rounded-xl font-bold transition-all duration-300 flex items-center justify-center gap-2"
          >
            <svg className="w-5 h-5 fill-current" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
              <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z" />
            </svg>
            Escribir por WhatsApp
          </a>
          <a
            href="tel:+573137804820"
            className="text-gray-400 hover:text-white border border-white/10 px-6 py-3 rounded-xl font-medium transition-all duration-300"
          >
            +57 313 780 4820
          </a>
        </div>
      </div>
    </section>
  );
}
